<template>
  <div class="top-level">
    <div class="bottom">
      ...
      <div>
        <b-button type="is-primary is-light is-large" icon-left="setting"></b-button>
        <b-tooltip :type="onlineStatusType">
          <b-button :type='{"is-light": true, "is-large": true, ...onlineStatusType}' icon-left="server"></b-button>
          <template v-slot:content><b-icon :icon="isOnline ? 'wifi' : 'wifi-slash'" size="is-small" /> {{ isOnline ? 'Connected' : 'No connection'}}</template>
        </b-tooltip>
      </div>
        <b-tooltip>
          <b-button type="is-primary is-light is-medium" disabled icon-left="code-branch"> 2.0.0</b-button>
          <template v-slot:content><b-icon icon="heart" size="is-small" /> Developed by <b>mhw0</b></template>
        </b-tooltip>
