...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        // these refs hold the current state of the game, so that we can use it
        ...
        const currentExample: ComputedRef<Example | null> = computed<Example | null>(() => {
        ...
        const correctAnswersPercent = computed<number>(() => {
            const resultPercent = (v(correctAnswersCount) /
                    (v(correctAnswersCount) + v(incorrectAnswersCount))) *
                100;
            return resultPercent || 0; // FIXME: tofixed
        });
        ...
        function displayAttentionTexts() {
            ...
            const timerHandle = setInterval(() => {
                if (START_ATTENTION_TEXTS[currentAttentionTextIndex]) {
                    console.log("START_ATTENTION_TEXTS")
        ...
        function displayExamples() {
            ...
            if (!v(currentExample)) {
                console.log("there are no examples left")
                ...
                (function callback() {
                    ...
                    if (v(currentExample)!.numbers[currentRowIndex]) {
                        ...
                        if (props.answerAtEnd) {
                            ...
                            /* otherwise we just display the answer form */
                            displayAnswerForm();
                        }
        ...
        watch(currentExample, (value, oldValue, onInvalidate) => {
            console.log("currentExample", value, oldValue);
        })
